...
void state_t::reset(processor_t* const proc, reg_t max_isa)
{
  ...
  csrmap[CSR_HCONTEXT] = std::make_shared<masked_dtrig_csr_t>(proc, CSR_HCONTEXT, (reg_t(1) << hcontext_length) - 1, 0);
  csrmap[CSR_MCONTEXT] = mcontext = std::make_shared<proxy_dtrig_csr_t>(proc, CSR_MCONTEXT, csrmap[CSR_HCONTEXT]);
  debug_mode = false;
